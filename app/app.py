...
from models.database import Database, Car, PriceHistory, ScraperLog, User
...
def calculate_car_score(car, config, distance_weight=0.1):
    ...
    # BONUS 2: PHEV/Hybrid is a good contender
    if is_phev_or_hybrid:
        score -= 5  # Good bonus!
...
def index():
    ...
    stats = {
        ...
        'newest_car': session.query(func.max(Car.last_seen)).filter(Car.is_available == True).scalar()
    }
